...
class CookieListTable extends \WP_List_Table {
	...
	public function get_columns(): array {
		return array(
			...
			'name'        => __( 'Name', 'privaro-cookie-consent-banner' ),
			'domain'      => __( 'Domain', 'privaro-cookie-consent-banner' ),
			'category'    => __( 'Category', 'privaro-cookie-consent-banner' ),
			'service'     => __( 'Service', 'privaro-cookie-consent-banner' ),
			'duration'    => __( 'Duration', 'privaro-cookie-consent-banner' ),
			'detected_at' => __( 'Detected', 'privaro-cookie-consent-banner' ),
		);
	}
	...
	public function column_default( $item, $column_name ): string {
		switch ( $column_name ) {
			case 'name':
				...
				$actions = array(
					'edit'   => sprintf( '<a href="%s">%s</a>', $edit_url, __( 'Edit', 'privaro-cookie-consent-banner' ) ),
					'delete' => sprintf( '<a href="%s" onclick="return confirm(\'%s\')">%s</a>', $delete_url, __( 'Are you sure?', 'privaro-cookie-consent-banner' ), __( 'Delete', 'privaro-cookie-consent-banner' ) ),
				);
			...
			case 'detected_at':
				return '0000-00-00 00:00:00' === $item['detected_at'] ? __( 'Manual', 'privaro-cookie-consent-banner' ) : mysql2date( get_option( 'date_format' ), $item['detected_at'] );
	...
	public function extra_tablenav( $which ) {
		...
		<div class="alignleft actions">
			<select name="category" id="filter-by-category">
				<option value=""><?php esc_html_e( 'All Categories', 'privaro-cookie-consent-banner' ); ?></option>
			...
			<?php submit_button( __( 'Filter', 'privaro-cookie-consent-banner' ), '', 'filter_action', false, array( 'id' => 'post-query-submit' ) ); ?>
		</div>
		<?php
	}
