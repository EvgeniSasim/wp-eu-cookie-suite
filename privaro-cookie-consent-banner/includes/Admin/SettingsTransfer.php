...
final class SettingsTransfer {
	...
	public static function validate( $data ) {
		...
		if ( ! isset( $data['settings'] ) || ! is_array( $data['settings'] ) ) {
			return new \WP_Error( 'wpeu_cs_missing_settings', __( 'Export is missing settings data.', 'privaro-cookie-consent-banner' ) );
		}

		if ( ! isset( $data['registry'] ) || ! is_array( $data['registry'] ) ) {
			return new \WP_Error( 'wpeu_cs_missing_registry', __( 'Export is missing script registry data.', 'privaro-cookie-consent-banner' ) );
		}
