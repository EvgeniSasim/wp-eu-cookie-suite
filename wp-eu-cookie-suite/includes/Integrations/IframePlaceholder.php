...
final class IframePlaceholder {
	 ...
	 * @return string
	 */
	private function handle_iframe_tag( array $matches ): string {
		$tag = $matches[0];
		$src = '';
		...
		if ( $category && ! wpeu_cs_user_has_consent( $category ) ) {
			return $this->get_placeholder( $tag, $service_id, $category );
		}
	 ...
	 * @return string
	 */
	private function get_placeholder( string $original_tag, string $service_id, string $category ): string {
		$services = ScriptRegistry::get_services();
		...
		$text = sprintf(
			...
			__( 'Please accept %s cookies to view this content.', 'wp-eu-cookie-suite' ),
			'<strong>' . esc_html( $label ) . '</strong>'
		);

		$button_text = __( 'Accept Cookies', 'wp-eu-cookie-suite' );
