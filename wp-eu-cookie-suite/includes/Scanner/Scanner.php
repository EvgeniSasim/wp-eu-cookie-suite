...
final class Scanner {
	...
	public function ajax_get_scan_urls(): void {
		...
		if ( time() - $last_scan < MINUTE_IN_SECONDS ) {
			wp_send_json_error( array( 'message' => __( 'Please wait at least one minute between scans.', 'wp-eu-cookie-suite' ) ) );
		}
	...
	public function ajax_scan_url(): void {
		...
		if ( empty( $url ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid URL.', 'wp-eu-cookie-suite' ) ) );
		}
	...
	private function scan_url( string $url ): array {
		$response = wp_remote_get(
			...
			array(
				'timeout'    => 15,
				'user-agent' => 'WP EU Cookie Suite Scanner',
			)
		);
