...
final class SettingsTransfer {
	...
	public static function validate( $data ) {
		if ( ! is_array( $data ) ) {
			return new \WP_Error( 'wpeu_cs_invalid_json', __( 'Invalid JSON structure.', 'wp-eu-cookie-suite' ) );
		}

		if ( ( $data['plugin'] ?? '' ) !== 'wp-eu-cookie-suite' ) {
			return new \WP_Error( 'wpeu_cs_invalid_plugin', __( 'This file is not a WP EU Cookie Suite export.', 'wp-eu-cookie-suite' ) );
		}

		if ( ! isset( $data['settings'] ) || ! is_array( $data['settings'] ) ) {
			return new \WP_Error( 'wpeu_cs_missing_settings', __( 'Export is missing settings data.', 'wp-eu-cookie-suite' ) );
		}

		if ( ! isset( $data['registry'] ) || ! is_array( $data['registry'] ) ) {
			return new \WP_Error( 'wpeu_cs_missing_registry', __( 'Export is missing script registry data.', 'wp-eu-cookie-suite' ) );
		}
