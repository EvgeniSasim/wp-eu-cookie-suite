...
final class Admin {
	...
	private function handle_json_export(): void {
		...
		if ( false === $json ) {
			wp_die( esc_html__( 'Could not encode export data.', 'eu-cookie-consent-suite' ) );
		}

		header( 'Content-Type: application/json; charset=utf-8' );
		header( 'Content-Disposition: attachment; filename=eu-cookie-consent-suite-' . gmdate( 'Y-m-d' ) . '.json' );
	...
	private function handle_download_snapshot( int $id ): void {
		...
		if ( ! $log || empty( $log['config_snapshot'] ) ) {
			wp_die( esc_html__( 'Snapshot not found.', 'eu-cookie-consent-suite' ) );
		}
	...
	public function add_menu_page(): void {
		add_menu_page(
			__( 'EU Cookie Consent', 'eu-cookie-consent-suite' ),
			__( 'EU Cookie Consent', 'eu-cookie-consent-suite' ),
			'manage_options',
			self::PAGE_SLUG,
	...
	public function render_settings_page(): void {
		...
		$tabs = array(
			'dashboard'    => __( 'Dashboard', 'eu-cookie-consent-suite' ),
			'banner'       => __( 'Banner', 'eu-cookie-consent-suite' ),
			'cookies'      => __( 'Cookies', 'eu-cookie-consent-suite' ),
			'scanner'      => __( 'Scanner', 'eu-cookie-consent-suite' ),
			'consent_log'  => __( 'Consent Log', 'eu-cookie-consent-suite' ),
			'integrations' => __( 'Integrations', 'eu-cookie-consent-suite' ),
			'tools'        => __( 'Tools', 'eu-cookie-consent-suite' ),
		);

		$active_tab_input = isset( $_GET['tab'] ) ? sanitize_key( wp_unslash( (string) $_GET['tab'] ) ) : '';
		$active_tab       = array_key_exists( $active_tab_input, $tabs ) ? $active_tab_input : 'dashboard';
	...
	private function render_language_selector( string $active_tab, array $locales, string $current_lang ): void {
		...
		<div class="wpeu-cs-lang-selector">
			<ul class="subsubsub">
				...
				foreach ( $locales as $code => $label ) :
					...
					if ( ! $is_core ) {
						echo ' <a href="' . esc_url( $remove_url ) . '" class="wpeu-cs-remove-lang" onclick="return confirm(\'' . esc_js( __( 'Are you sure you want to remove this language? Settings for this language will be deleted.', 'eu-cookie-consent-suite' ) ) . '\');" title="' . esc_attr__( 'Remove language', 'eu-cookie-consent-suite' ) . '"><span class="dashicons dashicons-no-alt" style="font-size: 16px; width: 16px; height: 16px; vertical-align: middle;"></span></a>';
					}
		...
		<div class="wpeu-cs-add-lang-form card">
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=' . self::PAGE_SLUG ) ); ?>">
				...
				<h4 style="margin-top: 0;"><?php esc_html_e( 'Add Language', 'eu-cookie-consent-suite' ); ?></h4>
				<div style="display: flex; gap: 10px; align-items: flex-end;">
					<div>
						<label for="new_lang_code" style="display: block; font-size: 11px;"><?php esc_html_e( 'Code (e.g. fr)', 'eu-cookie-consent-suite' ); ?></label>
						<input type="text" name="wpeu_cs_new_lang_code" id="new_lang_code" value="" class="small-text" required maxlength="5">
					</div>
					<div>
						<label for="new_lang_label" style="display: block; font-size: 11px;"><?php esc_html_e( 'Display Name', 'eu-cookie-consent-suite' ); ?></label>
						<input type="text" name="wpeu_cs_new_lang_label" id="new_lang_label" value="" class="regular-text" required placeholder="Français">
					</div>
					<?php submit_button( __( 'Add', 'eu-cookie-consent-suite' ), 'secondary', 'submit', false ); ?>
				</div>
			</form>
		</div>
		<?php
	...
	private function render_categories_management(): void {
		...
		$integration_opts = array(
			Categories::PREFERENCES => __( 'Preferences', 'eu-cookie-consent-suite' ),
			Categories::STATISTICS  => __( 'Statistics', 'eu-cookie-consent-suite' ),
			Categories::MARKETING   => __( 'Marketing', 'eu-cookie-consent-suite' ),
		);
		?>
		<h3><?php esc_html_e( 'Categories', 'eu-cookie-consent-suite' ); ?></h3>
		<p class="description"><?php esc_html_e( 'Built-in categories are always available. Add up to 5 custom categories for site-specific consent groups.', 'eu-cookie-consent-suite' ); ?></p>

		<table class="widefat striped" style="max-width: 900px;">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Slug', 'eu-cookie-consent-suite' ); ?></th>
					<th><?php esc_html_e( 'Label', 'eu-cookie-consent-suite' ); ?></th>
					<th><?php esc_html_e( 'Counts as', 'eu-cookie-consent-suite' ); ?></th>
					<th><?php esc_html_e( 'Type', 'eu-cookie-consent-suite' ); ?></th>
					<th></th>
				</tr>
			</thead>
			<tbody>
				<?php foreach ( $builtin as $slug => $category ) : ?>
					<tr>
						...
						<td><?php esc_html_e( 'Built-in', 'eu-cookie-consent-suite' ); ?></td>
						<td></td>
					</tr>
				<?php endforeach; ?>
				<?php foreach ( $custom as $slug => $category ) : ?>
					...
					<tr>
						...
						<td><?php esc_html_e( 'Custom', 'eu-cookie-consent-suite' ); ?></td>
						<td>
							<a href="<?php echo esc_url( $remove_url ); ?>" class="submitdelete" onclick="return confirm('<?php echo esc_js( __( 'Remove this custom category?', 'eu-cookie-consent-suite' ) ); ?>');"><?php esc_html_e( 'Remove', 'eu-cookie-consent-suite' ); ?></a>
						</td>
					</tr>
				<?php endforeach; ?>

				<tr>
					<th scope="row"><?php esc_html_e( 'Reload on revoke', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label class="switch">
							<input type="checkbox" name="wpeu_cs_settings[reload_on_revoke]" value="1" <?php checked( ! empty( $settings['reload_on_revoke'] ) ); ?>>
							<span class="slider round"></span>
						</label>
						<p class="description"><?php esc_html_e( 'Reload the page immediately after consent is revoked.', 'eu-cookie-consent-suite' ); ?></p>
					</td>
				</tr>
			</tbody>
		</table>
		...
		<div class="wpeu-cs-add-category-form card" style="margin-top: 12px; max-width: 900px;">
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=' . self::PAGE_SLUG ) ); ?>">
				...
				<h4 style="margin-top: 0;"><?php esc_html_e( 'Add Custom Category', 'eu-cookie-consent-suite' ); ?></h4>
				<table class="form-table" role="presentation" style="margin-top: 0;">
					<tr>
						<th scope="row"><label for="wpeu_cs_new_category_slug"><?php esc_html_e( 'Slug', 'eu-cookie-consent-suite' ); ?></label></th>
					...
					<tr>
						<th scope="row"><label for="wpeu_cs_new_category_label"><?php esc_html_e( 'Label', 'eu-cookie-consent-suite' ); ?></label></th>
						<td><input type="text" name="wpeu_cs_new_category_label" id="wpeu_cs_new_category_label" class="regular-text" required placeholder="<?php esc_attr_e( 'Social Media', 'eu-cookie-consent-suite' ); ?>"></td>
					</tr>
					<tr>
						<th scope="row"><label for="wpeu_cs_new_category_description"><?php esc_html_e( 'Description', 'eu-cookie-consent-suite' ); ?></label></th>
					...
					<tr>
						<th scope="row"><label for="wpeu_cs_new_category_integration_map"><?php esc_html_e( 'Counts as (for blocking integrations)', 'eu-cookie-consent-suite' ); ?></label></th>
						<td>
				...
				<?php submit_button( __( 'Add Category', 'eu-cookie-consent-suite' ), 'secondary' ); ?>
			</form>
		</div>
		<?php endif; ?>
		<hr>
	...
	private function render_banner_tab(): void {
		...
		if ( 'lang_added' === $message ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Language added successfully.', 'eu-cookie-consent-suite' ) . '</p></div>';
		} elseif ( 'lang_removed' === $message ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Language removed.', 'eu-cookie-consent-suite' ) . '</p></div>';
		} elseif ( 'invalid_code' === $message ) {
			echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__( 'Invalid language code. Use 2-5 characters.', 'eu-cookie-consent-suite' ) . '</p></div>';
		} elseif ( 'category_added' === $message ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Custom category added.', 'eu-cookie-consent-suite' ) . '</p></div>';
		} elseif ( 'category_removed' === $message ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Custom category removed.', 'eu-cookie-consent-suite' ) . '</p></div>';
		} elseif ( 'invalid_category_slug' === $message ) {
			echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__( 'Invalid category slug. Use 2-32 lowercase letters, numbers, hyphens or underscores. Built-in slugs are reserved.', 'eu-cookie-consent-suite' ) . '</p></div>';
		} elseif ( 'category_limit' === $message ) {
			echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__( 'Maximum number of custom categories reached.', 'eu-cookie-consent-suite' ) . '</p></div>';
		} elseif ( 'category_exists' === $message ) {
			echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__( 'A category with this slug already exists.', 'eu-cookie-consent-suite' ) . '</p></div>';
		} elseif ( 'invalid_integration_map' === $message ) {
			echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__( 'Invalid integration map value.', 'eu-cookie-consent-suite' ) . '</p></div>';
		}
		...
		<form method="post" action="options.php">
			...
			<table class="form-table" role="presentation">
				<tr>
					...
					<td>
						...
						<?php endforeach; ?>
						<p class="description"><?php esc_html_e( 'Select which consent categories to display in the banner.', 'eu-cookie-consent-suite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Privacy Policy URL', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<input type="url" name="wpeu_cs_settings[privacy_policy_url]" value="<?php echo esc_url( $privacy_url ); ?>" class="regular-text">
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Cookie Policy URL', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<input type="url" name="wpeu_cs_settings[cookie_policy_url]" value="<?php echo esc_url( $cookie_url ); ?>" class="regular-text">
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Show "Reject All"', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="wpeu_cs_settings[show_reject_all]" value="1" <?php checked( $show_reject_all ); ?>>
							<?php esc_html_e( 'Show the "Reject All" button in the banner.', 'eu-cookie-consent-suite' ); ?>
						</label>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Strict EU Mode', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="wpeu_cs_settings[eu_mode]" value="1" <?php checked( $eu_mode ); ?>>
							<?php esc_html_e( 'Enable strict EU mode (block all non-necessary cookies until consent).', 'eu-cookie-consent-suite' ); ?>
						</label>
					</td>
				</tr>
			</table>
			...
			<hr>
			<h3><?php esc_html_e( 'Localized Texts', 'eu-cookie-consent-suite' ); ?></h3>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Consent Modal Title', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Consent Modal Description', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Preferences Modal Title', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Accept All Button', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Reject All Button', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Manage Preferences Button', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Save Preferences Button', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Revoke Consent Label', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Close Icon Label', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<?php foreach ( $all_categories as $id => $category ) : ?>
					...
					?>
					<tr>
						<th scope="row"><?php echo esc_html( $category['label'] ); ?> (<?php esc_html_e( 'Label', 'eu-cookie-consent-suite' ); ?>)</th>
						<td>
					...
					<tr>
						<th scope="row"><?php echo esc_html( $category['label'] ); ?> (<?php esc_html_e( 'Description', 'eu-cookie-consent-suite' ); ?>)</th>
						<td>
			...
			<hr>
			<h3><?php esc_html_e( 'Appearance', 'eu-cookie-consent-suite' ); ?></h3>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Layout', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<select name="wpeu_cs_settings[banner_ui][layout]" id="wpeu-cs-banner-layout">
							<option value="box" <?php selected( $layout, 'box' ); ?>><?php esc_html_e( 'Box', 'eu-cookie-consent-suite' ); ?></option>
							<option value="bar" <?php selected( $layout, 'bar' ); ?>><?php esc_html_e( 'Bar', 'eu-cookie-consent-suite' ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Position', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<select name="wpeu_cs_settings[banner_ui][position]" id="wpeu-cs-banner-position">
							<option value="bottom-left" <?php selected( $position, 'bottom-left' ); ?>><?php esc_html_e( 'Bottom Left', 'eu-cookie-consent-suite' ); ?></option>
							<option value="bottom-center" <?php selected( $position, 'bottom-center' ); ?>><?php esc_html_e( 'Bottom Center', 'eu-cookie-consent-suite' ); ?></option>
							<option value="bottom-right" <?php selected( $position, 'bottom-right' ); ?>><?php esc_html_e( 'Bottom Right', 'eu-cookie-consent-suite' ); ?></option>
							<option value="center" <?php selected( $position, 'center' ); ?>><?php esc_html_e( 'Center', 'eu-cookie-consent-suite' ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Theme', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<select name="wpeu_cs_settings[banner_ui][theme]" id="wpeu-cs-banner-theme">
							<option value="light" <?php selected( $theme, 'light' ); ?>><?php esc_html_e( 'Light', 'eu-cookie-consent-suite' ); ?></option>
							<option value="dark" <?php selected( $theme, 'dark' ); ?>><?php esc_html_e( 'Dark', 'eu-cookie-consent-suite' ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Primary Color', 'eu-cookie-consent-suite' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Custom CSS', 'eu-cookie-consent-suite' ); ?></th>
					<td>
			...
			<div class="wpeu-cs-preview-container">
				<h3><?php esc_html_e( 'Live Preview', 'eu-cookie-consent-suite' ); ?></h3>
				...
				<p>
					<button type="button" id="wpeu-cs-refresh-preview" class="button button-secondary"><?php esc_html_e( 'Refresh Preview', 'eu-cookie-consent-suite' ); ?></button>
				</p>
			</div>
	...
	private function render_consent_log_tab(): void {
		...
		<div class="wpeu-cs-consent-log-header">
			<h2><?php esc_html_e( 'Consent Log', 'eu-cookie-consent-suite' ); ?></h2>
			<p class="description"><?php esc_html_e( 'Audit trail of visitor consent interactions. This is a technical aid for Art. 7(1) GDPR accountability.', 'eu-cookie-consent-suite' ); ?></p>
		</div>

		<form method="get">
			<input type="hidden" name="page" value="<?php echo esc_attr( self::PAGE_SLUG ); ?>">
			<input type="hidden" name="tab" value="consent_log">
			<?php $table->search_box( __( 'Search Logs', 'eu-cookie-consent-suite' ), 'search-id' ); ?>
			<?php $table->display(); ?>
		</form>
		<?php
	}
	...
	private function render_dashboard_tab(): void {
		...
		<div class="wpeu-cs-dashboard-cards">
			<div class="wpeu-cs-card">
				<h3><?php esc_html_e( 'Plugin Version', 'eu-cookie-consent-suite' ); ?></h3>
				<p><?php echo esc_html( $version ); ?></p>
			</div>

			<div class="wpeu-cs-card">
				<h3><?php esc_html_e( 'Blocker Status', 'eu-cookie-consent-suite' ); ?></h3>
				<p>
					<span class="status <?php echo $blocker ? 'status-active' : 'status-inactive'; ?>">
						<?php echo $blocker ? esc_html__( 'Active', 'eu-cookie-consent-suite' ) : esc_html__( 'Inactive', 'eu-cookie-consent-suite' ); ?>
					</span>
				</p>
			</div>

			<div class="wpeu-cs-card">
				<h3><?php esc_html_e( 'Consent API Status', 'eu-cookie-consent-suite' ); ?></h3>
				<p>
					<span class="status status-active">
						<?php
						if ( defined( 'WP_CONSENT_API_VERSION' ) ) {
							esc_html_e( 'Active (native)', 'eu-cookie-consent-suite' );
						} else {
							esc_html_e( 'Active (polyfill)', 'eu-cookie-consent-suite' );
						}
						?>
					</span>
				</p>
			...
			<div class="wpeu-cs-card">
				<h3><?php esc_html_e( 'Active Block Rules', 'eu-cookie-consent-suite' ); ?></h3>
				<p><?php echo (int) $total_rules; ?></p>
			</div>

			<div class="wpeu-cs-card">
				<h3><?php esc_html_e( 'Logs (Last 30 days)', 'eu-cookie-consent-suite' ); ?></h3>
	...
	private function render_integrations_tab(): void {
		...
		<form method="post" action="options.php">
			...
			<h2><?php esc_html_e( 'Google', 'eu-cookie-consent-suite' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Google Consent Mode v2', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="wpeu_cs_settings[google_consent_mode]" value="1" <?php checked( $google_gcm ); ?>>
							<?php esc_html_e( 'Send default denied consent to gtag before analytics/marketing tags load.', 'eu-cookie-consent-suite' ); ?>
						</label>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Google Analytics cookie guard', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label class="switch">
						...
						<p class="description"><?php esc_html_e( 'Clears _ga cookies and blocks enqueued gtag scripts when statistics consent is not granted.', 'eu-cookie-consent-suite' ); ?></p>
					</td>
				</tr>
				<?php if ( defined( 'GOOGLESITEKIT_VERSION' ) ) : ?>
				<tr>
					<th scope="row"><?php esc_html_e( 'Google Site Kit', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label class="switch">
						...
						<p class="description"><?php esc_html_e( 'Block Site Kit analytics tags until statistics consent (works with Consent Mode v2 above).', 'eu-cookie-consent-suite' ); ?></p>
					</td>
				</tr>
				<?php endif; ?>
			</table>

			<h2><?php esc_html_e( 'Third-Party Integrations', 'eu-cookie-consent-suite' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Theme Analytics (ACF)', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label class="switch">
						...
						<p class="description"><?php esc_html_e( 'Intercept ACF option field for analytics.', 'eu-cookie-consent-suite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'ACF Analytics Field Name', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<input type="text" name="wpeu_cs_settings[theme_analytics_field]" value="<?php echo esc_attr( $analytics_field ); ?>" class="regular-text">
						<p class="description"><?php esc_html_e( 'The name of the ACF field used for analytics/tracking code.', 'eu-cookie-consent-suite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Iframe Placeholders', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label class="switch">
						...
						<p class="description"><?php esc_html_e( 'Replace YouTube, Vimeo, and Google Maps iframes with placeholders.', 'eu-cookie-consent-suite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Contact Form 7 reCAPTCHA', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label class="switch">
						...
						<p class="description"><?php esc_html_e( 'Only load reCAPTCHA scripts after marketing consent.', 'eu-cookie-consent-suite' ); ?></p>
					</td>
				</tr>
			</table>

			<h2><?php esc_html_e( 'Service Blocker', 'eu-cookie-consent-suite' ); ?></h2>
			<p><?php esc_html_e( 'Enable automatic script blocking for these popular services.', 'eu-cookie-consent-suite' ); ?></p>

			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Script blocker enabled', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						<label class="switch">
							<input type="checkbox" name="wpeu_cs_settings[blocker_enabled]" value="1" <?php checked( $blocker_enabled ); ?>>
							<span class="slider round"></span>
						</label>
						<p class="description"><?php esc_html_e( 'Output-buffer blocking for third-party scripts matched by the registry and custom rules.', 'eu-cookie-consent-suite' ); ?></p>
					</td>
				</tr>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Custom Block Rules', 'eu-cookie-consent-suite' ); ?></th>
					<td>
						...
						<p class="description">
							<?php esc_html_e( 'Enter one pattern per line to block custom scripts. Use -url- prefix to match only the src attribute.', 'eu-cookie-consent-suite' ); ?><br>
							<?php esc_html_e( 'Example: analytics.js or -url-my-script.js', 'eu-cookie-consent-suite' ); ?>
						</p>
					</td>
				</tr>
			</table>
	...
	private function render_scanner_tab(): void {
		...
		<div class="wpeu-cs-scanner-container">
			<h2><?php esc_html_e( 'Cookie Scanner', 'eu-cookie-consent-suite' ); ?></h2>
			<p><?php esc_html_e( 'Scan your website to detect cookies and third-party scripts.', 'eu-cookie-consent-suite' ); ?></p>

			<div class="wpeu-cs-scanner-actions">
				<button type="button" id="wpeu-cs-start-scan" class="button button-primary">
					<?php esc_html_e( 'Start Scan', 'eu-cookie-consent-suite' ); ?>
				</button>
				...
				<?php if ( ! empty( $results['cookies'] ) || ! empty( $results['scripts'] ) ) : ?>
					<div class="wpeu-cs-scan-actions-secondary">
						<button type="button" id="wpeu-cs-import-scan" class="button button-secondary">
							<?php esc_html_e( 'Import to Inventory', 'eu-cookie-consent-suite' ); ?>
						</button>
						<span class="spinner" style="float: none; margin-top: 0;"></span>
					</div>
				<?php endif; ?>
	...
	public function render_scan_results_table( array $results, array $categories ): void {
		if ( empty( $results['cookies'] ) && empty( $results['scripts'] ) ) {
			echo '<p class="wpeu-cs-no-results">' . esc_html__( 'No scan results found. Start a scan to find cookies and scripts.', 'eu-cookie-consent-suite' ) . '</p>';
			return;
		}

		?>
		<h3><?php esc_html_e( 'Detected Items', 'eu-cookie-consent-suite' ); ?></h3>
		<table class="wp-list-table widefat fixed striped">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Name / Domain', 'eu-cookie-consent-suite' ); ?></th>
					<th><?php esc_html_e( 'Type', 'eu-cookie-consent-suite' ); ?></th>
					<th><?php esc_html_e( 'Detected Category', 'eu-cookie-consent-suite' ); ?></th>
				</tr>
			</thead>
			<tbody>
	...
	private function render_cookies_tab(): void {
		...
		<form method="get">
			...
			<?php $table->search_box( __( 'Search Cookies', 'eu-cookie-consent-suite' ), 'search-id' ); ?>
			<?php $table->display(); ?>
		</form>
		<?php
	}
	...
	private function render_cookie_form( int $id = 0 ): void {
		...
		$title = $id ? __( 'Edit Cookie', 'eu-cookie-consent-suite' ) : __( 'Add New Cookie', 'eu-cookie-consent-suite' );
		?>
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=' . self::PAGE_SLUG . '&tab=cookies' ) ); ?>">
			...
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="cookie_name"><?php esc_html_e( 'Name', 'eu-cookie-consent-suite' ); ?></label></th>
					<td><input type="text" name="cookie[name]" id="cookie_name" value="<?php echo esc_attr( $cookie['name'] ?? '' ); ?>" class="regular-text" required></td>
				</tr>
				<tr>
					<th scope="row"><label for="cookie_domain"><?php esc_html_e( 'Domain', 'eu-cookie-consent-suite' ); ?></label></th>
					<td><input type="text" name="cookie[domain]" id="cookie_domain" value="<?php echo esc_attr( $cookie['domain'] ?? '' ); ?>" class="regular-text"></td>
				</tr>
				<tr>
					<th scope="row"><label for="cookie_category"><?php esc_html_e( 'Category', 'eu-cookie-consent-suite' ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="cookie_service"><?php esc_html_e( 'Service', 'eu-cookie-consent-suite' ); ?></label></th>
					<td><input type="text" name="cookie[service]" id="cookie_service" value="<?php echo esc_attr( $cookie['service'] ?? '' ); ?>" class="regular-text"></td>
				</tr>
				<tr>
					<th scope="row"><label for="cookie_duration"><?php esc_html_e( 'Duration', 'eu-cookie-consent-suite' ); ?></label></th>
					<td><input type="text" name="cookie[duration]" id="cookie_duration" value="<?php echo esc_attr( $cookie['duration'] ?? '' ); ?>" class="regular-text"></td>
				</tr>
				<tr>
					<th scope="row"><label for="cookie_description"><?php esc_html_e( 'Description', 'eu-cookie-consent-suite' ); ?></label></th>
		...
		<p>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . self::PAGE_SLUG . '&tab=cookies' ) ); ?>">
				&larr; <?php esc_html_e( 'Back to Inventory', 'eu-cookie-consent-suite' ); ?>
			</a>
		</p>
		<?php
	}
	...
	private function render_tools_tab(): void {
		...
		?>
		<h2><?php esc_html_e( 'Tools', 'eu-cookie-consent-suite' ); ?></h2>
		...
		if ( 'imported' === $message ) :
			?>
			<div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'Settings imported successfully.', 'eu-cookie-consent-suite' ); ?></p></div>
			<?php
		elseif ( 'import_invalid' === $message ) :
			?>
			<div class="notice notice-error is-dismissible"><p><?php esc_html_e( 'Import failed: invalid or incompatible export file.', 'eu-cookie-consent-suite' ); ?></p></div>
			<?php
		elseif ( 'import_error' === $message ) :
			?>
			<div class="notice notice-error is-dismissible"><p><?php esc_html_e( 'Import failed: could not read the uploaded file.', 'eu-cookie-consent-suite' ); ?></p></div>
			<?php
		elseif ( 'revision_bumped' === $message ) :
			?>
			<div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'Consent revision updated. Visitors will be prompted again.', 'eu-cookie-consent-suite' ); ?></p></div>
			<?php
		endif;
		?>

		<div class="notice notice-info inline">
			<p><strong><?php esc_html_e( 'Disclaimer:', 'eu-cookie-consent-suite' ); ?></strong> <?php esc_html_e( 'This plugin provides tools for cookie compliance but does not constitute legal advice.', 'eu-cookie-consent-suite' ); ?></p>
		</div>
		...
		<form method="post" action="options.php">
			...
			<div class="card">
				...
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="policy_intro"><?php esc_html_e( 'Policy Intro Text', 'eu-cookie-consent-suite' ); ?></label></th>
						<td>
							...
							<p class="description"><?php esc_html_e( 'This text is displayed at the beginning of your cookie policy.', 'eu-cookie-consent-suite' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="policy_template"><?php esc_html_e( 'Policy Template', 'eu-cookie-consent-suite' ); ?></label></th>
						<td>
							...
							<p class="description">
								<?php esc_html_e( 'The template for the [wpeu_cookie_policy] shortcode.', 'eu-cookie-consent-suite' ); ?><br>
								<?php esc_html_e( 'Available placeholders: {{intro}}, {{table}}, {{content}}', 'eu-cookie-consent-suite' ); ?>
							</p>
						</td>
					</tr>
				</table>
		...
		<div class="card">
			<h3><?php esc_html_e( 'Consent Logging', 'eu-cookie-consent-suite' ); ?></h3>
			<form method="post" action="options.php">
				<?php settings_fields( 'wpeu_cs_settings' ); ?>
				<input type="hidden" name="wpeu_cs_settings[active_tab]" value="tools">
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Enable Logging', 'eu-cookie-consent-suite' ); ?></th>
						<td>
							<label class="switch">
							...
							<p class="description"><?php esc_html_e( 'Store an audit trail of consent events in the local database.', 'eu-cookie-consent-suite' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Retention (days)', 'eu-cookie-consent-suite' ); ?></th>
						<td>
							...
							<p class="description"><?php esc_html_e( 'Automatically delete logs older than this many days.', 'eu-cookie-consent-suite' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Store IP Hash', 'eu-cookie-consent-suite' ); ?></th>
						<td>
							<label class="switch">
								<input type="checkbox" name="wpeu_cs_settings[consent_log_store_ip]" value="1" <?php checked( ! empty( $settings['consent_log_store_ip'] ) ); ?>>
								<span class="slider round"></span>
							</label>
							<p class="description"><?php esc_html_e( 'Store a salted SHA-256 hash of the visitor IP address for better accountability (Art. 7 GDPR).', 'eu-cookie-consent-suite' ); ?></p>
						</td>
					</tr>
				</table>
				<?php submit_button(); ?>
		...
		<div class="card">
			<h3><?php esc_html_e( 'Consent revision', 'eu-cookie-consent-suite' ); ?></h3>
			<p><?php esc_html_e( 'Increment the consent revision to re-prompt all visitors (existing consent cookies become outdated).', 'eu-cookie-consent-suite' ); ?></p>
			<p><strong><?php esc_html_e( 'Current revision:', 'eu-cookie-consent-suite' ); ?></strong> <?php echo (int) ( $settings['consent_revision'] ?? 0 ); ?></p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=' . self::PAGE_SLUG . '&tab=tools' ) ); ?>" onsubmit="return confirm('<?php echo esc_js( __( 'Reset all visitor consents and show the banner again?', 'eu-cookie-consent-suite' ) ); ?>');">
				...
				<?php submit_button( __( 'Reset all consents (bump revision)', 'eu-cookie-consent-suite' ), 'delete', 'submit', false ); ?>
			</form>
		</div>

		<div class="card">
			<h3><?php esc_html_e( 'Export Cookie Inventory', 'eu-cookie-consent-suite' ); ?></h3>
			<p><?php esc_html_e( 'Download your cookie inventory as a CSV file.', 'eu-cookie-consent-suite' ); ?></p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=' . self::PAGE_SLUG . '&tab=tools' ) ); ?>">
				...
				<?php submit_button( __( 'Download CSV', 'eu-cookie-consent-suite' ), 'primary', 'submit', false ); ?>
			</form>
		</div>

		<div class="card">
			<h3><?php esc_html_e( 'Export Settings', 'eu-cookie-consent-suite' ); ?></h3>
			<p><?php esc_html_e( 'Download banner texts, integrations, script registry, and all plugin settings as JSON for backup or migration to another site.', 'eu-cookie-consent-suite' ); ?></p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=' . self::PAGE_SLUG . '&tab=tools' ) ); ?>">
				<?php wp_nonce_field( 'wpeu_cs_export_json', 'wpeu_cs_export_json_nonce' ); ?>
				<input type="hidden" name="action" value="wpeu_cs_export_json">
				<?php submit_button( __( 'Download JSON', 'eu-cookie-consent-suite' ), 'secondary', 'submit', false ); ?>
			</form>
		</div>

		<div class="card">
			<h3><?php esc_html_e( 'Import Settings', 'eu-cookie-consent-suite' ); ?></h3>
			<p><?php esc_html_e( 'Upload a JSON export from another site. Cookie inventory is not replaced — only plugin settings are updated.', 'eu-cookie-consent-suite' ); ?></p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=' . self::PAGE_SLUG . '&tab=tools' ) ); ?>" enctype="multipart/form-data">
				...
				<?php submit_button( __( 'Import JSON', 'eu-cookie-consent-suite' ), 'secondary', 'submit', false ); ?>
			</form>
		</div>
		<?php
	}
	...
	private function render_placeholder_tab( string $task_id ): void {
		echo '<p>' . esc_html__( 'This section is not available yet.', 'eu-cookie-consent-suite' ) . '</p>';
	}
}
