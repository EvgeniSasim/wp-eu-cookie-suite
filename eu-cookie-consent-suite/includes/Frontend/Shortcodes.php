...
final class Shortcodes {
	...
	public function render_cookie_table( $atts ): string {
		...
		if ( empty( $cookies ) ) {
			return '<p>' . esc_html__( 'No cookies detected.', 'eu-cookie-consent-suite' ) . '</p>';
		}
			...
			<?php foreach ( $grouped as $cat_id => $cat_cookies ) : ?>
				...
				<div class="wpeu-cookie-category-section">
					...
					<table class="wpeu-cookie-table">
						<thead>
							<tr>
								<th><?php esc_html_e( 'Name', 'eu-cookie-consent-suite' ); ?></th>
								<th><?php esc_html_e( 'Domain', 'eu-cookie-consent-suite' ); ?></th>
								<th><?php esc_html_e( 'Service', 'eu-cookie-consent-suite' ); ?></th>
								<th><?php esc_html_e( 'Duration', 'eu-cookie-consent-suite' ); ?></th>
								<?php if ( $show_description ) : ?>
									<th><?php esc_html_e( 'Description', 'eu-cookie-consent-suite' ); ?></th>
								<?php endif; ?>
							</tr>
						</thead>
						<tbody>
	...
	public function render_revoke_consent( $atts ): string {
		...
		if ( '' === $label ) {
			...
			$label  = $texts['revoke_consent_label'] ?? __( 'Revoke cookie consent', 'eu-cookie-consent-suite' );
		}
	...
	public function render_manage_consent( $atts ): string {
		...
		if ( '' === $label ) {
			$locale = BannerTexts::get_active_locale();
			$texts  = BannerTexts::get_strings( $locale );
			$label  = $texts['manage_consent_label'] ?? __( 'Cookie settings', 'eu-cookie-consent-suite' );
		}
