...
final class Banner {
	...
	public function render_config(): void {
		...
		<script type="text/javascript">
			window.addEventListener('load', function () {
				...
				const logConsentFromState = function (consentData) {
					const categories = Object.keys(cc.getConfig().categories);
					let eventType = 'save_preferences';
					const allEnabled = categories.filter(function (c) {
						return !cc.getConfig().categories[c].readOnly;
					});
					const acceptedEnabled = allEnabled.filter(function (c) {
						return consentData[c];
					});
				...
				const ccConfig = <?php echo wp_json_encode( $config ); ?>;
				ccConfig.onConsent = function () {
					syncWpeuCookies();
				};
				ccConfig.onFirstConsent = function () {
					ensureWpeuUuid();
					const consentData = syncWpeuCookies();
					logConsentFromState(consentData);
				};
				ccConfig.onChange = function (event) {
					const consentData = syncWpeuCookies();
	...
	private function get_config(): array {
		...
		if ( ! empty( $privacy_url ) ) {
			$footer_links[] = '<a href="' . esc_url( $privacy_url ) . '">' . __( 'Privacy Policy', 'eu-cookie-consent-suite' ) . '</a>';
		}
		if ( ! empty( $cookie_url ) ) {
			$footer_links[] = '<a href="' . esc_url( $cookie_url ) . '">' . __( 'Cookie Policy', 'eu-cookie-consent-suite' ) . '</a>';
		}
