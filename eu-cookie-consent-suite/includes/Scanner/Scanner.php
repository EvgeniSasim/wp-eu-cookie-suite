...
final class Scanner {
	...
	public function ajax_get_scan_urls(): void {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions.', 'eu-cookie-consent-suite' ) ) );
		}

		$last_scan = get_option( 'wpeu_cs_last_scan_time', 0 );
		if ( time() - $last_scan < MINUTE_IN_SECONDS ) {
			wp_send_json_error( array( 'message' => __( 'Please wait at least one minute between scans.', 'eu-cookie-consent-suite' ) ) );
		}
	...
	public function ajax_scan_url(): void {
		...
		if ( empty( $url ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid URL.', 'eu-cookie-consent-suite' ) ) );
		}
	...
	private function scan_url( string $url ): array {
		$response = wp_remote_get(
			...
			array(
				'timeout'    => 15,
				'user-agent' => 'EU Cookie Consent Suite Scanner',
			)
		);
	...
	public function ajax_import_scan(): void {
		...
		if ( empty( $results ) ) {
			wp_send_json_error( array( 'message' => __( 'No scan results to import.', 'eu-cookie-consent-suite' ) ) );
		}
		...
		wp_send_json_success(
			array(
				...
				'message' => sprintf( __( 'Imported %d items to inventory.', 'eu-cookie-consent-suite' ), $count ),
			)
		);
	}
