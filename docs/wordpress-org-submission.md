...
- [ ] Проверьте, что версия в `wp-eu-cookie-suite.php` совпадает со `Stable tag` в `readme.txt` (сейчас `1.0.0`).
...
3. Загрузите файл `build/wp-eu-cookie-suite.zip`.
...
### Структура SVN:
- `/trunk`: Основная ветка разработки (копия содержимого папки `wp-eu-cookie-suite`).
...
1. Сделайте checkout репозитория:
   ```bash
   svn co https://plugins.svn.wordpress.org/wp-eu-cookie-suite/ my-plugin-svn
   ```
...
## Важные правила
- Используйте Text Domain `wp-eu-cookie-suite` для всех строк перевода.
